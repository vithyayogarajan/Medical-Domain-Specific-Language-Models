...
df = pd.read_csv("eICU_fungal42_data.csv")
...
MAX_LEN = 1250 ## maximum sequence length 
TRAIN_BATCH_SIZE = 1
VALID_BATCH_SIZE = 1
EPOCHS = 35
LEARNING_RATE = 1e-05
...
class LongformerClass(torch.nn.Module):
    def __init__(self):
        ...
        self.l1 = transformers.AutoModel.from_pretrained('allenai/longformer-base-4096')
...
f1_score_micro = []
f1_score_macro = []
myfile = open('evaluation_output_eachepoch_longformer_fungal_42label.txt', 'w')
for epoch in range(EPOCHS):
    ...
    myfile.write("\n")
    myfile.write("This is the output of eICU for multi-label problem Systemic fungal or bacterial infection with Longformer, max seq = 1250 \n")
...
with open('output_longformer_final_fungal_42label_fulldissummary.txt', 'w') as f:
        print("This is the output of eICU for multi-label problem Systemic fungal or bacterial infection with Longformer, max seq = 1250 ",file=f)
        print("   ",file=f)
